...
1️⃣ Clone the repository or download the dataset.  
2️⃣ Install dependencies using:  
   ```bash
   pip install -r requirements.txt
   ```  
3️⃣ Run the training script for different models:  
   ```bash
   python train.py --model ResNet
   ```  
4️⃣ Evaluate model performance using:  
   ```bash
   python evaluate.py --model ResNet
   ```  
5️⃣ Visualize results (accuracy, PR curves) using:  
   ```bash
